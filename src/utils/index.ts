...
export function findMatchKey(langObj: any, text: string) {
 ...
 */
export const getAllFiles = (dir: string): any =>
  fs.readdirSync(dir).reduce((files, file): any => {
 ...
 * 获取文件 Json
 */
export function getLangJson(fileName: string): any {
...
export const getConfiguration = (text: string) => {
  ...
  if (text in configObj) return configObj[text];

  let value = vscode.workspace.getConfiguration('i18n-esign').get(text);
  let kiwiConfigJson = getConfigFile();
  if (!kiwiConfigJson) return value;

  const config = getLangJson(kiwiConfigJson);
  if (text in config) value = config[text];
...
export const getConfigFile = () => {
  let kiwiConfigJson = `${vscode.workspace.rootPath}/.i18n-esign.js`;
  // 先找js
  if (!fs.existsSync(kiwiConfigJson)) {
    kiwiConfigJson = `${vscode.workspace.rootPath}/.i18n-esign.ts`;
    //再找ts
    if (!fs.existsSync(kiwiConfigJson)) return null;
  }
 ...
 * @param times
 */
function retry(asyncOperation: any, times = 1) {
  let runTimes = 1;
  const handleReject = (e: Error) => {
    if (runTimes++ < times) {
  ...
  function _translateText() {
    return withTimeout(
      new Promise((resolve, reject) => {
        googleTranslate(text, 'zh', 'en', (err: Error, translation: any) => {
          if (err) {
            reject(err);
          } else {
